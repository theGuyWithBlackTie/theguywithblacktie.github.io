...
We can combine multiple built-in modules into one bigger custom module. Each built-in module has `forward()` function similar to `PyTorch`'s `forward()`, and similarly we would need to implement `forward()` function in our custom module too. Below is the example of using multiple modules:
